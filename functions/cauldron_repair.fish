function cauldron_repair --description "Repair broken Cauldron installation"
    ...
    if string match -q "*Fish config*" -- $issues
        ...
        echo "set -gx CAULDRON_CONFIG_DIR \"\$HOME/.config/cauldron\"" >> "$fish_config"
        ...
        echo "set -gx CAULDRON_INTERNAL_TOOLS \"$CAULDRON_PATH/tools\"" >> "$fish_config"
        echo "" >> "$fish_config"
        echo "if not contains \"\$HOME/.config/cauldron/functions\" \$fish_function_path" >> "$fish_config"
        echo "    set -gx fish_function_path \$fish_function_path \"\$HOME/.config/cauldron/functions\"" >> "$fish_config"
